...
try{
    ...
    if($ExampleSettingsFileRemoved){
        ...
        write-output "ENABLE_AD_ADMIN_USER" 1>> $ExampleSettingsFile
...
if ((Get-Content $SETTINGS_CONFIG).Contains("ENABLE_AD_ADMIN_USER")){
...
if($ADCredentialFile.Length -lt 5){
    ...
    if(!($CredentialFileExists)){
        ...
        if($SetupActiveDirectoryCredentials){
            ...
            (Get-Credential).Password | ConvertFrom-SecureString | Out-File "$CredentialFile"
        }else{
            write-output "Credentials Needed for User: $ADAdminUser | On Server: $ADAdminServer"
            exit 1
        }
...
function Authenticate-To-ActiveDirectory-Server(){
    ...
    if(!($Success)){
        ...
        Write-Output "Successful Connection to Remote AD"
        Write-Output ""
    }
