...
try{
    $GroupObject = Get-ADGroup -Identity "$GroupGUIDIn"
    $GroupGUID = $GroupObject.ObjectGUID.ToString()
    $GroupName = $GroupObject.Name.ToString()
    $DistinguishedName = $GroupObject.DistinguishedName.ToString()
    $GroupOU = "OU="+($DistinguishedName -split ",OU=",2)[1]
...
Write-Output "Membership For Group [ $GroupName ]"
Write-Output "--------------------------------------------------------------------"
Get-ADGroupMember -Identity "$GroupGUIDIn" | Select-Object name, objectClass,distinguishedName | ft -HideTableHeaders
Write-Output "--------------------------------------------------------------------"
    ...
    if($FindAndShowAllUsersInContext){
        ...
        #Get-ADUser -Filter * -Searchbase $contextIn  -ErrorAction Stop | select Name,Givenname,Surname,sAMAccountName,distinguishedName  | fl 
            if(!($ExcludeGroupEmpty)){
                    ...
                    $GroupDN = (Get-ADGroup -Identity $AD_EXCLUDE_GROUP).DistinguishedName

                if($SkipUserSamAccountNameDisplay){
                    $Users = (Get-ADUser -filter {objectclass -eq "user"} -Searchbase $contextIn -ErrorAction Stop -properties memberof).where{$_.memberof -notcontains $GroupDN} | select Givenname,Surname  | Format-Table -HideTableHeaders
                }else{
                    $Users = (Get-ADUser -filter {objectclass -eq "user"} -Searchbase $contextIn -ErrorAction Stop -properties memberof).where{$_.memberof -notcontains $GroupDN} | select Givenname,Surname,SamAccountName  | Format-Table -HideTableHeaders
                }
                    $Users
            }else{
        ...
        try{
                if(!($ExcludeGroupEmpty)){
                        ...
                        if($SkipUserSamAccountNameDisplay){
                            $Users = (Get-ADUser -filter {objectclass -eq "user"} -ErrorAction Stop -properties memberof).where{$_.memberof -notcontains $GroupDN} | select Givenname,Surname  | Format-Table -HideTableHeaders
                        }else{
                            $Users = (Get-ADUser -filter {objectclass -eq "user"} -ErrorAction Stop -properties memberof).where{$_.memberof -notcontains $GroupDN} | select Givenname,Surname,SamAccountName  | Format-Table -HideTableHeaders
                        }
                            $Users
                    }else{
                        if($SkipUserSamAccountNameDisplay){
...
if(!($UserSearch)){
    ...
    if (!( $firstNameSet -and $lastNameSet)){ 
        ...
        if($NewUserTemplate.Length -gt 0 ){
            ...
            try{
                $NewUserTemplateObject = Get-ADUser -Identity $NewUserTemplate
            }catch{
                    Write-Output ""
...
try{
    $TheUserAccount = Get-ADUser -Identity "$SamAccountNameIn" -Properties * -ErrorAction Stop
}catch{
    return
}
...
try{
    $TheUser = Get-ADUser -Identity $SamAccountName -ErrorAction Stop 2> $null
}catch{
    $UserDoesNotExist = $true
}
...
try{
    Get-ADUser -Identity $NewUserTemplate -ErrorAction Stop | out-null 2> $null
}catch{
    $err = "$_"
...
try{
    $TemplateUserObjectAttributes = Get-ADUser -Identity "$NewUserTemplate" -Properties $SearchProperties
}catch{
    $err = "$_"
...
function Set-UserProperty {
    ...
    try{
        Set-ADUser $User @SetADUserParameters
    }catch{
        $ItDidNotWork = $true
    }
    ...
    if($ItDidNotWork){
        try{
            Set-ADUser -Identity $User -Replace $SetADUserParameters 
        }catch{}
    }
}
...
$TemplateUserAllProperties = Get-ADUser -Identity "$NewUserTemplate" -Properties *
...
if($TemplateUserObjectAttributes.Contains("HomeDirectory")){
    ...
    if(($TemplateUserHomeDirectory).Length -gt 0 ){
        if($TemplateUserHomeDirectory -clike "*\${TemplateUserSamAccountName}*"){
            ...
            Set-Aduser $SamAccountName -HomeDirectory "$NewHomeDirectory"
           }
    }
...
if(($TemplateUserHomeDrive).Length -gt 0 ){
    if($TemplateUserHomeDrive -clike "*\${TemplateUserSamAccountName}*"){
        ...
        Set-Aduser $SamAccountName -HomeDrive "$NewHomeDrive"
        }
}
...
if($DisableNewTemplateUserGroupMemberships){
    ...
    $GroupMembershipOfTemplateUser = Get-ADPrincipalGroupMembership -Identity "$NewUserTemplate"

    try{
        $GroupMembershipOfTemplateUser | foreach {  try{Add-ADPrincipalGroupMembership -Identity $SamAccountNameIn -MemberOf $_ -ErrorAction Ignore 1>$null 2>$null 3>$null | out-Null  }catch{}  }
    }catch{}
    ...
    if(!($AllowExcludeGroup)){
        ...
        try{
            Remove-ADGroupMember -Identity $AD_EXCLUDE_GROUP -Members $SamAccountNameIn -Confirm:$false -ErrorAction Ignore 1>$null 2>$null 3>$null | out-Null 
        }catch{}
    }
...
function Create-ActiveDirectory-Account(){
    ...
    $ProxyAddressArray.ForEach({ Set-ADUser -Identity "$SAM" -Add @{proxyaddresses="smtp:$_"} })
...
$createUserResult = $true
    ...
    if($PrimarySmtpAddress.Length -gt 3){
        try{
        Set-ADUser -Identity "$SAM" -remove @{proxyaddresses="smtp:$PrimarySmtpAddress"}
        }catch{}
        Set-ADUser -Identity "$SAM" -Add @{proxyaddresses="SMTP:$PrimarySmtpAddress"}
        $EmailSettingsChanged = $true
     }
    ...
    try{
        $GroupMembershipOfTemplateUser | foreach {  try{Add-ADPrincipalGroupMembership -Identity $SAM -MemberOf $_ -ErrorAction Ignore 1>$null 2>$null 3>$null | out-Null  }catch{}  }
    }catch{}
    ...
    if(!($AllowExcludeGroup)){
        ...
        try{
            Remove-ADGroupMember -Identity $AD_EXCLUDE_GROUP -Members $SAM -Confirm:$false -ErrorAction Ignore 1>$null 2>$null 3>$null | out-Null 
        }catch{}
    }
...
# Assign the mail property
    ...
    $modifyUserResult = $true
    try{
        Set-ADUser -Identity "$SamAccountNameIn" -EmailAddress $PrimarySmtpAddress -ErrorAction Stop 2> $null
    }catch{
        $modifyUserResult = $false
        $err = "$_"
    }
...
$ProxyAddress = $_ 
    ...
    if(@($ProxyAddress) -clike "`*SMTP`*"){
        Set-ADUser -Identity "$SamAccountNameIn" -remove @{ ProxyAddresses = "$ProxyAddress" }
        # Add the entry unless
        if(!($RemovePriorPrimarySmtpAddress)){
            $ProxyAddressOnly = $ProxyAddress.Split(":")[1]
            Set-ADUser -Identity "$SamAccountNameIn" -Add @{proxyaddresses="smtp:$ProxyAddressOnly"}
        }
...
Set-ADUser -Identity "$SamAccountNameIn" -remove @{ proxyaddresses="smtp:$PrimarySmtpAddress" }

Set-ADUser -Identity "$SamAccountNameIn" -Add @{ proxyaddresses="SMTP:$PrimarySmtpAddress" }
...
if((@( $TheUserProxyAddresses -like "`*${ProxyAddress}`*").Count) -eq 0 ){
    ...
    $modifyUserResult = $true
    try{
        Set-ADUser -Identity "$TheUserAccount" -Add @{proxyaddresses="smtp:$ProxyAddress"}  -ErrorAction Stop 2> $null
    }catch{
        $modifyUserResult = $false
        $err = "$_"
    }
...
$ProxyAddress = $_
        ...
        try{
            Set-ADUser -Identity "$TheUserAccount" -Remove @{proxyaddresses="smtp:$ProxyAddress"}  -ErrorAction Ignore 2> $null
        }catch{}
