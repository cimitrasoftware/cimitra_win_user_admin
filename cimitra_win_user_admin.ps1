...
try{
if ((Get-Content -ErrorAction SilentlyContinue $SETTINGS_CONFIG).Contains("ENABLE_AD_ADMIN_USER=True")){
...
if($ADCredentialFile.Length -lt 5){
    ...
    $SettingsFileNameOnly = Split-Path "$SETTINGS_CONFIG" -Leaf
    $SettingsFileName = $SettingsFileNameOnly -replace "\.", "_"
...
function Authenticate-To-ActiveDirectory-Server(){
    ...
    # Write-Output "CredentialFile = $CredentialFile"
...
if($SettingsFile.Length -lt 2){
    ...
    $Global:ConfigurationFile = $SettingsFile
}
...
$GroupObjectResult = $true

try{

        try{
        $GroupObject = Get-ADGroup -ErrorAction Stop @SrvConnect  -Identity "$GetGroupInfo"
    }catch{
        $GroupObjectResult = $false
    }

    if(!($GroupObjectResult)){

        Write-Output "Membership For Group"
        Write-Output "Group ID: $GetGroupInfo"
        Write-Output "Cannot Determine Membership Information On This Group"
        return
...
try{
    ...
    $GroupObjectResult = $true
    try{
        $GroupObject = Get-ADGroup -ErrorAction Stop @SrvConnect  -Identity "$GroupGUIDIn"
    }catch{
        $GroupObjectResult = $false
    }
    ...
    if(!($GroupObjectResult)){
        ...
        Write-Output "Info On Group"
        Write-Output "Group GUID: $GroupGUIDIn"
        Write-Output "Cannot Determine Information On This Group"
        return
...
Write-Output "Membership For Group [ $GroupName ]"
Write-Output "--------------------------------------------------------------------"
Get-ADGroupMember -ErrorAction SilentlyContinue @SrvConnect -Identity "$GroupGUIDIn" | Select-Object name, objectClass,distinguishedName | ft -HideTableHeaders
Write-Output "--------------------------------------------------------------------"


try{
    Get-ADGroup -ErrorAction SilentlyContinue @SrvConnect -Identity "$GroupGUIDIn" -Properties *
}catch{}


}

function GroupMembership-Report($GroupGUIDIn){

$ExcludeGroupEmpty = [string]::IsNullOrWhiteSpace($AD_EXCLUDE_GROUP)

$GroupGUIDInEmpty = [string]::IsNullOrWhiteSpace($GroupGUIDIn)

if($GroupGUIDInEmpty){
    return
}

$UseExcludeGroup = $false
...
# Determine if any users exist in the AD_EXCLUDE_GROUP
     if(!($ExcludeGroupEmpty)){

        try{
         $UsersInExludeGroup = (Get-ADGroupMember @SrvConnect -Identity "$AD_EXCLUDE_GROUP" | Select-Object SamAccountName)
         }catch{}
            if(($UsersInExludeGroup.Count) -gt 0){
...
$UsersInGroup = @()
     ...
     if($UseExcludeGroup){
        ...
        $AllUsersInGroup = (Get-ADGroupMember @SrvConnect -Identity "$GroupGUIDIn" | Select-Object name, SamAccountName)

        $UsersInExcludeGroup = (Get-ADGroupMember @SrvConnect -Identity "$AD_EXCLUDE_GROUP" | Select-Object SamAccountName)

        $AllUsersInGroup.ForEach({
        $CurrentUserObject = $_
        $CurrentSamAccountName = $_.SamAccountName

        if(!($UsersInExcludeGroup.SamAccountName.Contains("$CurrentSamAccountName"))){
             $UsersInGroup += $CurrentUserObject
            }
...
if($SkipUserSamAccountNameDisplay){
    ...
    $UsersInGroup | Select-Object name | ft -HideTableHeaders
}else{
    ...
    if($FindAndShowAllUsersInContext){
        ...
        #Get-ADUser -Filter * -Searchbase $contextIn  -ErrorAction Stop | select Name,Givenname,Surname,sAMAccountName,distinguishedName  | fl 
            if(!($ExcludeGroupEmpty)){
                ...
                if($SkipUserSamAccountNameDisplay){
                    $Users = (Get-ADUser @SrvConnect   -filter {objectclass -eq "user"} -Searchbase $contextIn -ErrorAction Stop -properties memberof).where{$_.memberof -notcontains $GroupDN} | select Givenname,Surname | Sort-Object -Property Givenname,Surname | Format-Table -HideTableHeaders
                }else{
                    $Users = (Get-ADUser @SrvConnect   -filter {objectclass -eq "user"} -Searchbase $contextIn -ErrorAction Stop -properties memberof).where{$_.memberof -notcontains $GroupDN} | select Givenname,Surname,SamAccountName | Sort-Object -Property Givenname,Surname  | Format-Table -HideTableHeaders
                }
                    $Users
            }else{

                if($SkipUserSamAccountNameDisplay){
                    Get-ADUser @SrvConnect   -Filter * -Searchbase $contextIn -ErrorAction Stop | select Givenname,Surname | Sort-Object -Property Givenname,Surname  | Format-Table -HideTableHeaders
                }else{
                    Get-ADUser @SrvConnect   -Filter * -Searchbase $contextIn -ErrorAction Stop | select Givenname,Surname,SamAccountName | Sort-Object -Property Givenname,Surname  | Format-Table -HideTableHeaders
                }
            }
     }catch{
        $runResult = $false
        ...
        try{
                if(!($ExcludeGroupEmpty)){
                        ...
                        if($SkipUserSamAccountNameDisplay){
                            $Users = (Get-ADUser @SrvConnect   -filter {objectclass -eq "user"} -ErrorAction Stop -properties memberof).where{$_.memberof -notcontains $GroupDN} | select Givenname,Surname | Sort-Object -Property Givenname,Surname  | Format-Table -HideTableHeaders
                        }else{
                            $Users = (Get-ADUser @SrvConnect   -filter {objectclass -eq "user"} -ErrorAction Stop -properties memberof).where{$_.memberof -notcontains $GroupDN} | select Givenname,Surname,SamAccountName | Sort-Object -Property Givenname,Surname  | Format-Table -HideTableHeaders
                        }
                            $Users
                    }else{
                        if($SkipUserSamAccountNameDisplay){
                            Get-ADUser @SrvConnect   -Filter * -ErrorAction Stop | select Givenname,Surname | Sort-Object -Property Givenname,Surname  | Format-Table -HideTableHeaders
                        }else{
                            Get-ADUser @SrvConnect   -Filter * -ErrorAction Stop | select Givenname,Surname,SamAccountName | Sort-Object -Property Givenname,Surname  | Format-Table -HideTableHeaders
                        }
                    }
        }catch{
        $runResult = $false
...
function Find-And-Show-Expired-Users(){
    ...
    if($FindAndShowExpiredUsersInContext){
        try{
            Search-ADAccount @SrvConnect  -SearchBase $contextIn -AccountExpired -UsersOnly -ErrorAction Stop |  Select-Object Name,Givenname,Surname,sAMAccountName,distinguishedName,AccountExpirationDate | Sort-Object -Property Givenname,Surname | fl
        }catch{
            $runResult = $false
            $err = "$_"
        }
        ...
        try{
            Search-ADAccount @SrvConnect  -AccountExpired -UsersOnly -ErrorAction Stop |  Select-Object Name,Givenname,Surname,sAMAccountName,distinguishedName,AccountExpirationDate | Sort-Object -Property Givenname,Surname | fl
        }catch{
            $runResult = $false
            $err = "$_"
        }
...
function Find-And-Show-Disabled-Users(){
    ...
    if($FindAndShowDisabledUsersInContext){
        ...
        try{
        Get-ADUser @SrvConnect   -Filter {(Enabled -eq $false)} -Searchbase $contextIn  -ErrorAction Stop | select Name,Givenname,Surname,sAMAccountName,distinguishedName | Sort-Object -Property Givenname,Surname  | fl 
        }catch{
        $runResult = $false
        $err = "$_"
        }
        ...
        try{
        Get-ADUser @SrvConnect   -Filter {(Enabled -eq $false)} -ErrorAction Stop | select Name,Givenname,Surname,sAMAccountName,distinguishedName | Sort-Object -Property Givenname,Surname  | fl 
        }catch{
        $runResult = $false
        $err = "$_"
        }
...
function Find-And-Show-NoLogon-Users(){
    ...
    if($FindAndShowNoLogonUsersInContext){
        ...
        try{
        Get-ADUser @SrvConnect   -SearchBase $contextIn -Filter {-not (lastlogontimestamp -like "*") -and -not (iscriticalsystemobject -eq $true)}  -ErrorAction Stop | select Name,Givenname,Surname,sAMAccountName,distinguishedName | Sort-Object -Property Givenname,Surname | fl 
        }catch{
        $runResult = $false
        $err = "$_"
        }
        ...
        try{
        Get-ADUser @SrvConnect   -Filter {-not (lastlogontimestamp -like "*") -and -not (iscriticalsystemobject -eq $true)}  -ErrorAction Stop | select Name,Givenname,Surname,sAMAccountName,distinguishedName | Sort-Object -Property Givenname,Surname  | fl 
        }catch{
        $runResult = $false
        $err = "$_"
        }
...
function Add-User-To-Groups(){
    ...
    $GroupObjectResult = $true
    try{
        $TheGroupName = Get-ADGroup @SrvConnect  -Identity $GroupGuid 
    }catch{
        $GroupObjectResult = $false
    }
...
$UserFound =  $true
    ...
    try{
        $TheGroups = Get-ADPrincipalGroupMembership -ErrorAction SilentlyContinue @SrvConnect  $SamAccountNameIn | select name | ft -HideTableHeaders | where{$_ -ne ""}
    }catch{
        $GroupQuery = $false
    }
...
if($DisableNewTemplateUserGroupMemberships){
    ...
    $GroupMembershipRead = $true

    $GroupMembershipOfTemplateUser = Get-ADPrincipalGroupMembership -ErrorAction SilentlyContinue @SrvConnect -Identity "$NewUserTemplate"
...
function Update-Created-User-Property($IDENTITY_IN,$AD_ATTRIBUTE_NAME,$AD_ATTRIBUTE_VALUE,$AD_ATTRIBUTE_LABEL)
{
    ...
    try{
        Set-ADUser @SrvConnect @parameterHash -ErrorAction Stop 2> $null
    }catch{
...
$modifyUserResult = $true
    ...
    # Modify the user
    try{
    Set-ADAccountPassword @SrvConnect  -Identity $SamAccountNameIn -Reset -NewPassword (ConvertTo-SecureString -AsPlainText ${TheUserPasswordIn} -Force) -ErrorAction Stop
    }catch{
    $modifyUserResult = $false
    $err = "$_"
    }
...
function Disable-User-Account()
{
    ...
    try{
    Disable-ADAccount @SrvConnect  -Identity $SamAccountNameIn  -ErrorAction Stop
     }catch{
     $modifyUserResult = $false
     $err = "$_"
     }
...
function Enable-User-Account()
{
    ...
    try{
    Enable-ADAccount @SrvConnect  -Identity $SamAccountNameIn  -ErrorAction Stop
     }catch{
     $modifyUserResult = $false
     $err = "$_"
     }
...
function Remove-User-Account-Lock()
{
    ...
    Unlock-ADAccount @SrvConnect -Identity $SamAccountNameIn -ErrorAction Stop
     }catch{
     $modifyUserResult = $false
     $err = "$_"
     }
...
function Remove-User-Expiration-Date()
{
    ...
    try{
        Clear-ADAccountExpiration @SrvConnect  -Identity $SamAccountNameIn -ErrorAction Stop
     }catch{
        $modifyUserResult = $false
        $err = "$_"
     }
...
function Set-Password()
{
    ...
    # Modify the user
    try{
    Set-ADAccountPassword @SrvConnect -Identity $SamAccountNameIn -Reset -NewPassword (ConvertTo-SecureString -AsPlainText ${TheUserPassword} -Force) -ErrorAction Stop
    }catch{
    $modifyUserResult = $false
    $err = "$_"
    }
...
function Remove-UserObject(){
    ...
    try{
    Remove-ADUser @SrvConnect  -Identity $TheUserDistinguisedName -Confirm:$False -ErrorAction Stop
    }catch{
...
function Set-User-ExpirationDate(){
    ...
    if ($modifyUserResult)
    {
    $ExpirationDateHuman = Get-Date "${ExpirationDate}"
    Write-Output "User: ${UserFirstName} ${UserLastName} | Expire Date Changed To: $ExpirationDateHuman"
    }else{
    Write-Output ""
...
if($ExpireUserObject -and (!($RemoveExpirationDate))){
