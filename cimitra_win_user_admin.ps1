...
function Find-And-Show-Expired-Users(){
    ...
    if($FindAndShowExpiredUsersInContext){
        try{
            Search-ADAccount @SrvConnect  -SearchBase $contextIn -AccountExpired -UsersOnly -ErrorAction Stop |  Select-Object Name,Givenname,Surname,sAMAccountName,distinguishedName,AccountExpirationDate | fl
        }catch{
            $runResult = $false
            $err = "$_"
        }
        ...
        try{
            Search-ADAccount @SrvConnect  -AccountExpired -UsersOnly -ErrorAction Stop |  Select-Object Name,Givenname,Surname,sAMAccountName,distinguishedName,AccountExpirationDate | fl
        }catch{
            $runResult = $false
            $err = "$_"
        }
